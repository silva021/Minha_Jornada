...
fun FrequencyFilter(
    ...
    selectedItem: ReminderFrequency,
    onItemSelected: (ReminderFrequency) -> Unit
) {
    Row(
        modifier = modifier
        ...
        ReminderFrequency.entries.forEach { item ->
            FilterChip(
...
fun FrequencyFilterPreview() {
    FrequencyFilter(
        selectedItem = ReminderFrequency.WEEKDAYS,
        onItemSelected = {}
    )
}